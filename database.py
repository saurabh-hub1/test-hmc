...
def get_all_applications():
    ...
    for app in all_apps:
        ...
        result.append({
            ...
            'check_in_date': app.get('check_in_date'),
            'check_out_date': app.get('check_out_date'),
...
# 🔴 FIXED: get_pending_checkins function - ab app_id properly return karega
def get_pending_checkins():
    ...
    for app in pending:
        result.append({
            'app_id': str(app['_id']),  # ✅ Yeh line important hai!
            '_id': str(app['_id']),
            'applicant_name': app.get('applicant_name', 'N/A'),
            'mobile': app.get('mobile', 'N/A'),
...
def warden_check_out(app_id):
    ...
    if allocated_room:
        rooms.update_one(
            {'room_number': int(allocated_room)},
            {'$set': {
                'status': 'available',
                'current_application_id': None
            }}
